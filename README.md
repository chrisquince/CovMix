...
- **snv_plot.pdf**: Plot showing abundance of snv as predicted using varscan as a function of  snv abundance generated by predicted proportion from the proportion.tsv file. The legend "observed only once" refers to case where more than sample is run at the same time. If A snv is unique to a sample it is flaged as "observed only once". Otherwise it will be "not predicted" or "removed by threshold". 
 
 ![alt tag](./figs/snv_proportions.png)
